...
class Model(nn.Module):
    ...
    def _encode_and_decode_chunk(
        self, audio: mx.array, max_tokens: int, temperature: float,
    ) -> Tuple[str, int]:
        """Encode + decode a single audio chunk in batch mode."""
        if audio.ndim == 1:
            audio = audio[None, :]
        ...
        limit = min(max_tokens, int(math.ceil(dur * self.config.max_tokens_per_second)))
        cfg = self.config
        ...
        next_tok = logits.argmax() if temperature <= 0 else mx.random.categorical(logits / temperature)
        mx.eval(next_tok)
    ...
    def generate(self, audio, *, max_tokens: int = 500, temperature: float = 0.0,
        ...
        Audio is processed in ``chunk_duration``-second segments through
        the batch encoder (full sliding-window attention per chunk).
        Each chunk is encoded and decoded independently.

        For real-time streaming use the ``create_stream`` /
        ``add_audio`` / ``transcribe`` API instead.
        """
        ...
        for offset in range(0, total_samples, chunk_samples):
            ...
            text, n_tok = self._encode_and_decode_chunk(chunk, max_tokens, temperature)
